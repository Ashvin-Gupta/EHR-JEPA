...
python scripts/encode_text_embeddings.py \
    ...
    --output_path      "${DATA_DIR}/code_embeddings.pt" \
    --model_name       emilyalsentzer/Bio_ClinicalBERT \
    --batch_size       64 \
    --device           cpu \
    --labitems_file    "${MIMIC_DIR}/hosp/d_labitems.csv.gz" \
    --diagnoses_file   "${MIMIC_DIR}/hosp/d_icd_diagnoses.csv.gz" \
    --procedures_file  "${MIMIC_DIR}/hosp/d_icd_procedures.csv.gz"
