...
Quadrant (0 = attend, -inf = block):
  - context × context: all allowed
  - context × target: blocked
  - target × context: all allowed
  - target × target: diagonal only

With CLS additionally:
  - CLS → CLS, CLS → context: allowed; CLS → target: blocked
  - context → CLS: allowed (context may read CLS)
  - target → CLS: allowed (masks may read CLS)
"""
...
def build_causal_single_quadrant_mask_batch(
    ...
    *,
    ...
    for b in range(B):
        out[b] = build_causal_single_quadrant_mask(
            int(lengths_c[b]),
            int(lengths_t[b]),
