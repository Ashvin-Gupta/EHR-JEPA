...
**Branch B + `causal_single`:** target encoder on `[CLS | events]`; context prefix re-encoded; predictor input is **compact** `[CLS | context_enc | learnable MASK@future]` (RoPE: CLS at 0, events at original indices). Each slot gets `+ HoursSinceFirstEmbedding(hours_since_first)` from the collator (wall-clock hours since the first window event; MASK slots also use `mask_token`). `predictor.causal_single_attn`: `bidirectional`, `quadrant`, or `partial_causal` (latter two: CLS/context cannot attend to MASK slots; MASK→CLS+context; target↔target diagonal; `partial_causal` uses a causal lower-triangular top-left block on `[CLS | context]`). **Downstream** probe / supervised CLS uses **encoder** `[CLS | events]` from the target pathway, not predictor outputs. Optional time-decay on target tokens. Invalid rows skipped before the predictor batch.
