...
def test_supervised_cls_forward_backward():
    ...
    model = SupervisedCLSClassifier(
        emb, enc, cls_token, head_type="linear", head_dropout=0.0, pooling_mode="cls"
    )
    B, L = 2, 10
    codes = torch.randint(0, 31, (B, L))
    attn = torch.ones(B, L, dtype=torch.long)
...
def test_supervised_cls_mean_pool_forward_backward():
    d = 16
    emb = _tiny_vocab_embedding(d)
    enc = _tiny_encoder(d)
    model = SupervisedCLSClassifier(
        emb, enc, cls_token=None, head_type="linear", head_dropout=0.0, pooling_mode="mean_pool"
    )
    B, L = 2, 10
    codes = torch.randint(0, 31, (B, L))
    attn = torch.ones(B, L, dtype=torch.long)
...
def test_bert_supervised_forward_backward():
    ...
    model = BERTSupervisedClassifier(
        bert, head_type="linear", head_dropout=0.0, pooling_mode="cls"
    )
    B, L = 2, 8
...
def test_ar_supervised_forward_backward():
    d = 16
    vocab_size = 20
    emb = EventEmbedding(
        EmbeddingConfig(
    ...
    model = ARSupervisedClassifier(
        ar, head_type="linear", head_dropout=0.0, pooling_mode="cls"
    )
    B, L = 2, 8
    codes = torch.randint(0, vocab_size - 1, (B, L))
    attn = torch.ones(B, L, dtype=torch.long)
